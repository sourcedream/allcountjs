    ...
    fieldRenderingServiceProvider.defineFields(["$filter", "$compile", "$locale", "lcApi", "messages", function ($filter, $compile, $locale, rest, messages) {
        ...
        return {
            ...
            select: [function (value, fieldDescription) {
                ...
                var options = [];
                opts.map(op =>options.push({name: op}));
                ...
                return $compile('<select ng-model="selectValue"  ng-options="r.name as r.name for r in optionsValues" class="form-control"></select>')(scope);
            }]
        }
    }]);
